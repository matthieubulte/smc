...
We start by stating the following assumptions first proposed by Stuart \cite{stuart_2010}
...
\begin{proof}
  Let $\P_0(\d y) = \rho(y)\d y$ and $\P(\d y|u) = \rho(y - \G (u))\d y$. Since both measures have a Radon-Nikodym derivative with respect to the Lebesgue measure $\d y$, we have

  \begin{equation*}
    \frac{\d \P}{\d \P_0}(y|u) = \frac{\d \P}{\d \lambda}(y|u)\left(\frac{\d \P_0}{\d \lambda}(y)\right)^{-1} = \frac{\rho(y - \G (u)}{\rho(y)} = C(y)\rho(y - \G (u)),
  \end{equation*}
...
\begin{definition} Let $\mu$ and $\mu'$ be two measures, both absolutely continuous to an arbitrary measure $\nu$. The \textit{Hellinger distance} between $\mu$ and $\mu'$ is then defined as
...
\begin{proof}
  ...
  Let $y, y' \in Y$ and  $\mu^y, \mu^{y'}$ be the measures obtained by application of Bayes' rule and $Z, Z'$ be the respective model evidences. We start by showing that both $Z$ and $Z'$ are bounded by above and below by positive constants. Using Assumption \ref{assumption-ll}(2), and $r > \max\{\norm{y}_Y, \norm{y'}_Y\}$
  \begin{equation*}
    Z = \int_X\exp(-\Phi(u; y))\d\mu_0(u) \ge \int_{\norm{u}_X \le r}\exp(-C)\d\mu_0(u) = \exp(-C)\mu_0(\norm{u}_X \le r) > 0,
  \end{equation*}
  where the last inequality holds because $\mu_0$ has support equal to $X$. Similarly, using $\alpha$ from the light-tailedness of $\mu_0$ and Assumtion \ref{assumption-ll}(1) with $\epsilon = \alpha$, we obtain
  \begin{align*}
    Z &= \int_X\exp(-\Phi(u; y))\d\mu_0(u)\\
      &\le \int_X\exp(-C + \alpha\norm{u}_X^2)\d\mu_0(u)\\
      &= \exp(-C)\int_X\exp(\alpha\norm{u}_X^2)\d\mu_0(u) < \infty.
  \end{align*}
  We thus have constant positive and finite lower and upper bounds on $Z$, which can be analogously derived for $Z'$. Further, using the notation $a \land b = \min\{a, b\}$ and that the exponential function has Lipschitz constant $1$, we have for any $u \in X$
  \begin{align*}
  ...
  From the definition of the Hellinger distance, we 

  \begin{equation*}
    \begin{aligned}
      2d_{Hell}(\mu^y, \mu^{y'})^2 &= \int_X\bigg[Z^{-1/2}\exp\left(-\frac12\Phi(u;y)\right) - (Z')^{-1/2}\exp\left(-\frac12\Phi(u; y')\right)\bigg]^2\d\mu_0(u)\\
  ...
  Additionally, using the notation $a \land b = \min \{a, b\}$, we have
